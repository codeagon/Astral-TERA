...
function Theme(client) {
  ...
  this.active = {}
  this.default = { background: '#222', f_high: '#fff', f_med: '#ccc', f_low: '#999', f_inv: '#fff', b_high: '#888', b_med: '#666', b_low: '#444', b_inv: '#000' }
  ...
  this.save = (data) => {
    fs.writeFile(path.join(__dirname, 'theme.json'), JSON.stringify(data, null, 4), (err) => {
  ...
  this.load = (data) => {
    ...
    this.save(JSON.stringify(theme))
    this.active = theme
    if (this.onLoad) {
      this.onLoad(data)
    }
